// Send the Compounding Guide PDF to all bot users.
import fs from 'fs';
...
const PDF = path.join(__dirname, 'ebook', 'Compounding-Guide-10x-AI-Part1.pdf');
...
const send = async (chatId) => {
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      form.append('document', new Blob([fs.readFileSync(PDF)], { type: 'application/pdf' }), 'Compounding-Guide-10x-AI-Part1.pdf');
...
db.exec('CREATE TABLE IF NOT EXISTS guide_pdf_sent (telegram_id INTEGER PRIMARY KEY, sent_at TEXT)');
const sentSet = new Set(db.prepare('SELECT telegram_id FROM guide_pdf_sent').all().map(r => r.telegram_id));
...
for (let i = 0; i < users.length; i += CHUNK) {
  ...
  const results = await Promise.all(chunk.map(async u => {
    ...
    if (r === 'ok') db.prepare('INSERT OR IGNORE INTO guide_pdf_sent (telegram_id, sent_at) VALUES (?, ?)').run(u.telegram_id, new Date().toISOString());
    return r;
  }));
