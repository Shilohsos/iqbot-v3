...
import { createGiveawayEvent, activateGiveaway, activatePromoCode, activateMarathon, participate as giveawayParticipate, claimPromoCode, getMarathonLeaderboard, checkMarathonDeadlines, recordTrade as giveawayRecordTrade, selectWinners as giveawaySelectWinners, getActiveGiveaways, getGiveawayEvents, getGiveawayEvent, processUpdateQueue, processNotificationsQueue, } from './giveaway.js';
...
import { getAdminId, adminKeyboard, adminBackKeyboard, broadcastTargetKeyboard, broadcastLinkKeyboard, broadcastActionKeyboard, broadcastTimerKeyboard, broadcastSendOrScheduleKeyboard, broadcastDelayKeyboard, scheduledBroadcastsKeyboard, tokenTierKeyboard, generateTokenKeyboard, topTradersAdminKeyboard, funnelKeyboard, memberManagementKeyboard, activationsKeyboard, giveawayTargetKeyboard, giveawayManagerKeyboard, giveawayTypeKeyboard, giveawayCriteriaKeyboard, giveawayScheduleKeyboard, activeGiveawaysKeyboard, promoScheduleKeyboard, marathonDurationKeyboard, marathonScheduleKeyboard, composeTopicKeyboard, composeResultKeyboard, composeDeliveryKeyboard, } from './ui/admin.js';
...
async function executeScheduledBroadcast(scheduled) {
    ...
    try {
        await bot.telegram.sendMessage(getAdminId(), msg, { parse_mode: 'Markdown' });
    }
    catch { }
}
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function getSsidForUser(telegramId) {
...
async function sendStartMenu(ctx) {
    ...
    if (telegramId === getAdminId()) {
        ...
        await ctx.reply(`🛡️ *Admin Dashboard*\n\n` +
            `👥 Users: ${stats.total} total | ✅ ${stats.approved} approved | ⏳ ${stats.pending} pending | 🔔 ${stats.manual} manual | ❌ ${stats.rejected} rejected`, { parse_mode: 'Markdown', reply_markup: adminKeyboard() });
        return;
    }
    const user = getUser(telegramId);
...
async function startOnboarding(ctx) {
    // L1 — welcome
    try {
        await ctx.replyWithPhoto(ASSET('L1.png'));
    }
    catch { }
    ...
    try {
        await ctx.replyWithPhoto(ASSET('L3.png'));
    }
    catch { }
    await ctx.reply(`Connect your IQ Option account.\n\n` +
        ...
        `Pick what fits 👇`, { reply_markup: onboardKeyboard() });
}
async function askAccountConnection(ctx) {
    await ctx.reply(`Connect your IQ Option account.\n\n` +
        `Free signup · 60 seconds · Linked instantly.\n` +
        `Bot trades on your account. Money stays yours.\n\n` +
        `Pick what fits 👇`, { reply_markup: onboardKeyboard() });
    ...
    try {
        ...
        for (let round = 1; round <= effectiveRounds + 1; round++) {
            ...
            if (result.status === 'WIN' || result.status === 'TIE') {
                ...
                const winReply = await ctx.reply(`🏆 +$${result.pnl.toFixed(2)} added to your balance.\n\n` +
                    (round > 1 ? `Recovery complete.\n\n` : '') +
                    `💸 You just made +$${result.pnl.toFixed(2)}`, { reply_markup: { inline_keyboard: [[{ text: '🔄 New Opportunity', callback_data: 'ui:trade' }]] } });
...
bot.action('onboard:yes', async (ctx) => {
    ...
    await ctx.reply(`🔢 Enter your IQ Option User ID\n\n` +
        `How to find it:\n` +
        `Open IQ Option → Profile → copy the numeric User ID 🆔\n\n` +
...
bot.action('onboard:no', async (ctx) => {
    await ctx.answerCbQuery();
    const chatId = ctx.chat.id;
    const existing = onboardSessions.get(chatId) ?? { step: 'create_user_id' };
...
bot.action('onboard:autocreate', async (ctx) => {
    await ctx.answerCbQuery();
    const chatId = ctx.chat.id;
    const existing = onboardSessions.get(chatId) ?? { step: 'auto_create_email' };
...
bot.action(/^amt:(.+)$/, async (ctx) => {
    ...
    if (val === 'custom') {
        ...
        try {
            await ctx.editMessageText('⏱ Pick your expiry timeframe 👇\n⏱ Faster timeframes settle quicker.\n🐢 Longer timeframes ride bigger moves.', { reply_markup: timeframeKeyboard(tfBtnUser?.tier ?? undefined) });
        }
        catch { }
    }
});
...
bot.action(/^tf:(\d+)$/, async (ctx) => {
    ...
    try {
        const m = await ctx.replyWithPhoto(ASSET('L6.png'));
        state.lastImageMsgId = m.message_id;
    }
    catch { }
    ...
    try {
        await ctx.editMessageText(picksMsg, { reply_markup: pairKeyboard(0, tfTier) });
    }
    catch { }
});
...
bot.action(/^pair:(.+)$/, async (ctx) => {
    ...
    if (!ssid) {
        await ctx.reply('❌ Not connected. Use /connect to link your IQ Option account.');
        return;
    }
    ...
    try {
        ...
        if (!tierCfg.allowedTimeframes.includes(timeframe)) {
            ...
            await ctx.telegram.editMessageText(chatId, progressMsg.message_id, undefined, `⚠️ ${tfLabel(timeframe)} is not available on the ${tierCfg.label} tier. Upgrade for access.`).catch(() => { });
            return;
        }
...
bot.action('upsell:live', async (ctx) => {
    await ctx.answerCbQuery();
    const chatId = ctx.chat.id;
    const state = { step: 'amount', mode: 'live' };
    try {
        const m = await ctx.replyWithPhoto(ASSET('L5.png'));
        state.lastImageMsgId = m.message_id;
    }
...
bot.action('upsell:demo', async (ctx) => {
    await ctx.answerCbQuery();
    const chatId = ctx.chat.id;
    const state = { step: 'amount', mode: 'demo' };
    try {
        const m = await ctx.replyWithPhoto(ASSET('L5.png'));
        state.lastImageMsgId = m.message_id;
    }
...
bot.action('ui:help', async (ctx) => {
    ...
    await ctx.reply(`❓ *Help & FAQ*\n\n` +
        ...
        `*How to withdraw?*\nAll funds stay in your IQ Option account — withdraw directly from there.`, { parse_mode: 'Markdown', reply_markup: backKeyboard() });
});
...
bot.command('admin', async (ctx) => {
    ...
    if (!sub) {
        const stats = getApprovalStats();
        await ctx.reply(`🛡️ *Admin Panel*\n\n👥 ${stats.total} users | ✅ ${stats.approved} approved | ⏳ ${stats.pending} pending | 🔔 ${stats.manual} manual | ❌ ${stats.rejected} rejected`, { parse_mode: 'Markdown', reply_markup: adminKeyboard() });
        return;
    }
    if (sub === 'users') {
...
bot.action('admin:back', async (ctx) => {
    ...
    const stats = getApprovalStats();
    await ctx.reply(`🛡️ *Admin Dashboard*\n\n` +
        `👥 Users: ${stats.total} | ✅ ${stats.approved} | ⏳ ${stats.pending} | 🔔 ${stats.manual} | ❌ ${stats.rejected}`, { parse_mode: 'Markdown', reply_markup: adminKeyboard() });
});
// ─── Module 1: Today ─────────────────────────────────────────────────────────
...
bot.on('text', async (ctx) => {
            ...
            try {
                ...
                if (as.step === 'broadcast_message') {
                    try {
                        ...
                        let targetIds;
                        if (target === 'active')
                            targetIds = getActiveTraderIds(5);
                        else if (target === 'inactive')
                            targetIds = getInactiveTraderIds(5);
                        else
    ...
    if (upgradeSessions.has(chatId)) {
        ...
        if (useToken(tokenInput, ctx.from.id)) {
            ...
            await ctx.reply(`✅ Token accepted! Your tier has been upgraded to *${result.tier}*. 🎉`, { parse_mode: 'Markdown', reply_markup: startKeyboard(result.tier) });
        }
        else {
    ...
    if (ob) {
        if (ob.step === 'user_id' || ob.step === 'create_user_id') {
            ...
            try {
                ...
                if (result.found) {
                    ...
                    await ctx.reply('⏳ We were not able to confirm your User ID.\n\n' +
                        'Please consider creating a new account the right way using our link 👇👾\n\n' +
                        'You can as-well contact admin 👇💜', { reply_markup: { inline_keyboard: [
                    ...
                    try {
                        ...
                        await bot.telegram.sendMessage(getAdminId(), `🔔 *Manual approval needed*\nTelegram: ${userTag}\nIQ User ID: \`${iqUserId}\`\n\nApprove: /admin approve ${ctx.from.id}\nReject: /admin reject ${ctx.from.id}`, { parse_mode: 'Markdown' });
                    }
                    catch { }
                }
            }
                ...
                try {
                    const userTag2 = ctx.from.username
                        ? `@${escapeMd(ctx.from.username)}`
                        : `[User](tg://user?id=${ctx.from.id})`;
                    await bot.telegram.sendMessage(getAdminId(), `🔔 *Manual approval needed* (auto-check unavailable)\nTelegram: ${userTag2}\nIQ User ID: \`${iqUserId}\`\n\nApprove: /admin approve ${ctx.from.id}\nReject: /admin reject ${ctx.from.id}`, { parse_mode: 'Markdown' });
                }
                catch { }
            }
            return;
        ...
        if (ob.step === 'auto_create_email') {
            ...
            onboardSessions.delete(chatId);
            await ctx.reply('✅ Got it! We\'ve received your email.\n\n' +
            ...
            try {
                const userTag = ctx.from.username
                    ? `@${escapeMd(ctx.from.username)}`
                    : `[User](tg://user?id=${ctx.from.id})`;
                await bot.telegram.sendMessage(getAdminId(), `🤖 *Auto-account request*\nTelegram: ${userTag}\nEmail: \`${escapeMd(text)}\`\n\nManually create an IQ Option account, then DM the user the credentials and approve: /admin approve ${ctx.from.id}`, { parse_mode: 'Markdown' });
            }
            catch { }
        ...
        if (ob.step === 'connect_password' && ob.email) {
            ...
            try {
                ...
                if ((ob.loginFailCount ?? 0) >= 2) {
                    onboardSessions.delete(chatId);
                    await ctx.reply('Seems you\'re having trouble logging into your IQ Options Account 👾😨\n\nNo worries we\'re here to assist you. Contact admin below 👇💜', { reply_markup: { inline_keyboard: [[{ text: '👾 Contact admin', url: ADMIN_CONTACT_LINK }]] } });
                }
                else {
                    ob.step = 'connect_email';
                    ob.email = undefined;
    ...
    if (wiz.lastImageMsgId) {
        try {
            await ctx.telegram.deleteMessage(chatId, wiz.lastImageMsgId);
        }
        catch { }
    }
    try {
        const m = await ctx.replyWithPhoto(ASSET('L5.png'));
        wiz.lastImageMsgId = m.message_id;
    }
    catch { }
    const tfWizUser = getUser(ctx.from.id);
    await ctx.reply('⏱ Pick your expiry timeframe 👇\n⏱ Faster timeframes settle quicker.\n🐢 Longer timeframes ride bigger moves.', { reply_markup: timeframeKeyboard(tfWizUser?.tier ?? undefined) });
});
bot.catch((err, ctx) => {
