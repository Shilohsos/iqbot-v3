...
let wsProxyResolver;
export function setWsProxyResolver(fn) {
    wsProxyResolver = fn;
}
/**
...
export class BlitzOptions {
     ...
     */
    async refreshActives() {
        const response = await this.wsApiClient.doRequest(new CallBinaryOptionsGetInitializationDataV3());
    ...
    async buy(active, direction, expirationSize, price, balance) {
        ...
        const openWith = async (act) => {
            const request = new CallBinaryOptionsOpenBlitzOptionV2(act.id, direction, expirationSize, price, balance.id, act.profitPercent());
            return this.wsApiClient.doRequest(request);
        };
