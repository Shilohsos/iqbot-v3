...
const CURRENCY_SYMS = { NGN: '₦', EUR: '€', GBP: '£', USD: '$' };
...
export function timeframeKeyboard(_tier) {
    const labels = { 30: '30s', 60: '1m', 120: '2m', 300: '5m' };
    const row = ALL_TIMEFRAMES.map(s => ({ text: labels[s] ?? `${s}s`, callback_data: `tf:${s}` }));
    return {
        inline_keyboard: [
            row,
            [{ text: ' Cancel', callback_data: 'wizard:cancel' }],
...
export function pairKeyboard(page = 0, _tier) {
    const PAGE_SIZE = 6;
    const start = page * PAGE_SIZE;
    const pagePairs = ALL_PAIRS.slice(start, start + PAGE_SIZE);
    const rows = [];
    for (let i = 0; i < pagePairs.length; i += 2) {
        const row = [{ text: pairLabel(pagePairs[i]), callback_data: `pair:${pagePairs[i]}` }];
        if (pagePairs[i + 1]) {
    ...
    const navRow = [];
    if (start > 0)
        navRow.push({ text: '⟵️ Back', callback_data: `page:${page - 1}` });
    if (start + PAGE_SIZE < ALL_PAIRS.length)
        navRow.push({ text: 'More →️', callback_data: `page:${page + 1}` });
    if (navRow.length)
        rows.push(navRow);
...
export function signalPairKeyboard(page = 0) {
    const PAGE_SIZE = 6;
    const start = page * PAGE_SIZE;
    const pagePairs = ALL_PAIRS.slice(start, start + PAGE_SIZE);
    const rows = [];
    for (let i = 0; i < pagePairs.length; i += 2) {
        const row = [{ text: pairLabel(pagePairs[i]), callback_data: `spair:${pagePairs[i]}` }];
        if (pagePairs[i + 1]) {
    ...
    const navRow = [];
    if (start > 0)
        navRow.push({ text: '⟵️ Back', callback_data: `spage:${page - 1}` });
    if (start + PAGE_SIZE < ALL_PAIRS.length)
        navRow.push({ text: 'More →️', callback_data: `spage:${page + 1}` });
    if (navRow.length)
        rows.push(navRow);
...
export function signalTimeframeKeyboard(pair) {
    const labels = { 30: '30s', 60: '1m', 120: '2m', 300: '5m' };
    const row = ALL_TIMEFRAMES.map(s => ({ text: labels[s] ?? `${s}s`, callback_data: `stf:${s}` }));
    return {
        inline_keyboard: [
            row,
...
export function tfLabel(timeframeSec) {
    if (timeframeSec === 30)
        return '30s';
    if (timeframeSec === 60)
        return '1m';
...
export function affiliateFailKeyboard() {
...
export function demoUpsellKeyboard() {
    return {
        inline_keyboard: [
