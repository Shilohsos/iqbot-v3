import { getEnabledAutoMessages, getBroadcastTargetIds, markBroadcastSent, getTestUserId, getNextBroadcastAt, saveNextBroadcastAt, getMessageIndex, saveMessageIndex, getLastBroadcastMsgId, saveLastBroadcastMsgId, getConfig, getUser, } from './db.js';
import { resolveUsername } from './pidgin.js';
const RATE_LIMIT_MS = 50;
const INTERVAL_MS = 3_600_000; // 1 hour
const NAME_CACHE_TTL = 10 * 60 * 1000; // 10 min
const nameCache = new Map();
async function resolveName(bot, telegramId) {
...
async function fireBroadcast(bot) {
    ...
    for (const tid of targets) {
        try {
            ...
            if (prevMsgId) {
                try {
                    await bot.telegram.deleteMessage(tid, prevMsgId);
                }
                catch { }
            }
