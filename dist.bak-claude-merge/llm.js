...
${tone.styleGuide ? `\nSTYLE GUIDE (follow this precisely):\n${tone.styleGuide}` : ''}`;
    ...
    if (!raw)
        throw new Error('Deepseek returned an empty/malformed response');
    let content = raw;
    try {
...
export async function generateDiaryEntry(type, context) {
    let prompt = DIARY_PROMPTS[type];
    if (!prompt)
        throw new Error(`Unknown diary type: ${type}`);
