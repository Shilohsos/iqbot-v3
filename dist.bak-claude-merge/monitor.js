...
function checkPM2Status() {
    try {
        ...
        if (!bot)
            return 'Bot process not found in PM2';
        if (bot.pm2_env.status !== 'online')
            return `Bot status: ${bot.pm2_env.status}`;
        const restarts = bot.pm2_env.restart_time;
        if (lastKnownRestarts < 0)
            lastKnownRestarts = restarts; // seed on first check
        ...
        if (uptimeMs < 60_000)
            return `Bot uptime < 1 min (recent restart, ${restarts} restarts total)`;
        return null;
    }
    catch (err) {
...
function checkDatabase() {
    try {
        ...
        db.close();
        if (row.cnt < 0)
            return 'Database returned negative count';
        return null;
    }
    catch (err) {
...
function checkTrades(seconds = 300) {
    try {
        ...
        `).get(`-${seconds}`);
        db.close();
        if (!row || (row.total === 0 && row.errors === 0))
            return null;
        if (row.errors > 0) {
...
function checkDisk() {
    try {
        ...
        if (pct > 90)
            return `Disk usage: ${pct}% (CRITICAL)`;
        if (pct > 80)
            return `Disk usage: ${pct}%`;
        return null;
    }
    catch {
        return null;
...
function checkMemory() {
    try {
        ...
        const pct = parseInt(output);
        if (pct > 95)
            return `Memory usage: ${pct}% (CRITICAL)`;
        if (pct > 85)
            return `Memory usage: ${pct}%`;
        return null;
    }
    catch {
        return null;
...
// ─── LLM Analysis ────────────────────────────────────────────────────────────
async function analyzeWithLLM(errors) {
    if (!DEEPSEEK_API_KEY || errors.length === 0)
        return errors.join('\n');
...
async function runHealthCheck() {
    ...
    for (const { fn, component, level } of checks) {
        ...
        if (!result)
            continue;
        const effectiveLevel = result.includes('CRITICAL') ? 'critical' : level;
...
async function runLogAnalysis() {
    ...
    if (logErrorBuffer.length > 3)
        logErrorBuffer.shift();
