...
const RANK = { signals: 0, ai_trading: 1, auto_trading: 2 };
/** Normalize an arbitrary access_level string to a known Product. */
export function getProduct(accessLevel) {
    const key = accessLevel?.toLowerCase() ?? 'signals';
    if (key === 'auto_trading' || key === 'ai_trading')
        return key;
    return 'signals';
}
export function getProductConfig(accessLevel) {
    return PRODUCT_CONFIGS[getProduct(accessLevel)];
}
/** True when `have` grants at least the access of `need`. */
export function hasAccess(have, need) {
    return RANK[getProduct(have)] >= RANK[need];
}
/** Derive the access level a funded USD balance should unlock. */
export function getAccessLevel(fundedUsd) {
    if (fundedUsd >= AUTO_TRADING_MIN_USD)
        return 'auto_trading';
    if (fundedUsd >= AI_TRADING_MIN_USD)
        return 'ai_trading';
    return 'signals';
}
/**
 ...
 */
export function resolveAccess(fundedUsd, tokenGrant, accessExpiresAt) {
...
export function tierToAccess(tier) {
    const key = (tier ?? '').toUpperCase();
    if (key === 'PRO' || key === 'MASTER' || key === 'ADMIN')
        return 'ai_trading';
    return 'signals';
}
...
export function tokenToAccess(tokenValue) {
    const key = (tokenValue ?? '').toUpperCase();
    if (key === 'AUTO_TRADING' || key === 'MASTER')
        return 'auto_trading';
    if (key === 'AI_TRADING' || key === 'PRO')
        return 'ai_trading';
    return 'signals';
}
...
export function godModeStakePct(_balanceUsd) {
    return 0.05 + Math.random() * 0.05; // 5%–10%
}
/** Weighted timeframe shuffle: 50% 1m, 35% 30s, 15% 5m. */
export function godModeTimeframe(_balanceUsd) {
...
export function godModeGaleRounds(_balanceUsd, _stake) {
    return weightedRandom([
        { value: 6, weight: 40 },
        { value: 3, weight: 40 },
        { value: 0, weight: 20 },
 ...
 */
export function godModePickWorstAssets(count = 3) {
    const worstPool = [
...
export function clampDisplayConfidence(raw) {
...
export function martingaleWorstCase(stake, galeRounds) {
