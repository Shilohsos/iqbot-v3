...
import { executeTradeWithSdk } from './trade.js';
...
export function isH20(telegramId) {
    const u = getUser(telegramId);
    return u?.h20 === 1;
}
/** Called on bot startup — restart any H20 sessions that were interrupted. */
export function resumeH20Sessions() {
    const targets = db.prepare('SELECT telegram_id FROM users WHERE h20 = 1').all();
...
export function launchH20(telegramId) {
    if (running.has(telegramId))
        return;
    running.set(telegramId, startH20(telegramId));
}
async function startH20(telegramId) {
    ...
    try {
        ...
        logger.error('h20', `SDK create failed for ${telegramId}: ${e.message}`);
        running.delete(telegramId);
        return;
    }
    let pairIndex = 0;
    try {
        while (true) {
            ...
            try {
                ...
                const history = await candlesFacade.getCandles(active.id, H20_TIMEFRAME, { count: 200 });
                ...
                logger.warn('h20', `analysis failed for ${telegramId} ${pair}: ${e.message}`);
                continue;
            }
            const tradeReq = {
                pair,
            ...
            try {
                const result = await executeTradeWithSdk(sdk, tradeReq);
                ...
                db.prepare(`INSERT INTO trades (telegram_id, pair, direction, amount, status, pnl, trade_id, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`).run(telegramId, pair, direction, tradeAmount, result.status, result.pnl, result.tradeId ?? null);
            }
            catch (e) {
                logger.warn('h20', `trade error: ${e.message} (amount=$${tradeAmount.toFixed(2)}, pair=${pair})`);
            }
        ...
        logger.error('h20', `fatal error for ${telegramId}: ${e.message}`);
    }
    finally {
        try {
            await sdk.shutdown();
