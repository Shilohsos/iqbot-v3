...
import { getUser, getSessionTrades, insertNotification, getApprovedUsersWithTier, getTestUserId, createMarathonConfig, getActiveMarathonConfig, getMarathonConfig, setMarathonConfigStatus, getMarathonParticipant, insertMarathonParticipant, updateMarathonParticipantBalance, markMarathonParticipantQualified, markMarathonParticipantBlown, incrementMarathonPush, getActiveMarathonParticipants, getMarathonLeaderboard, } from './db.js';
...
export async function getUserLiveBalanceUsd(telegramId) {
...
export async function startMarathonBroadcast(giveawayId, minBalanceUsd, minTrades, minGrowth, telegram) {
    ...
    if (testUserId)
        console.log(`[test-mode] marathon broadcast only to test user ${testUserId}`);
    const msg = [
...
export async function handleMarathonJoin(telegramId, configId) {
...
export async function tickMarathonTracking(telegram) {
    ...
    for (const p of participants) {
        if (testUserId && p.telegram_id !== testUserId)
            continue;
...
async function sendPushMessage(telegram, telegramId, message, markup) {
    try {
...
export function getMarathonV2Leaderboard(configId) {
    return getMarathonLeaderboard(configId);
}
/** End marathon — mark config as ended. */
export function endMarathon(configId) {
