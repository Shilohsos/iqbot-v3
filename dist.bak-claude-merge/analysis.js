...
export async function analyzePairWithSdk(sdk, pair, timeframeSec, tier = 'DEMO', candleCount) {
    return runAnalysis(sdk, pair, timeframeSec, tier, candleCount);
}
export async function analyzePair(ssid, pair, timeframeSec, tier = 'DEMO', candleCount) {
...
async function runAnalysis(sdk, pair, timeframeSec, tier, candleCount) {
    ...
    if (history.length < Math.min(30, Math.max(2, Math.floor(count * 0.7))))
    ...
    if (candleCount !== undefined && candleCount <= 5 && candleCount >= 2) {
        ...
        const direction = rsi > 50 ? 'call' : 'put';
    ...
    if (upperTier === 'PRO' || upperTier === 'MASTER') {
        ...
        const direction = bullVotes >= 2 ? 'call' : 'put';
        const signals = [
    ...
    const direction = bullScore >= 50 ? 'call' : 'put';
