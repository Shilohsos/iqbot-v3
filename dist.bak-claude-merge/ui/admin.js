export function giveawayViewKeyboard(event) {
    const rows = [];
...
export function adminKeyboard(adminAnalysisAll = false) {
...
export function memberFilterKeyboard() {
    return {
        inline_keyboard: [
            [
...
export function userDetailKeyboard(telegramId) {
    return {
        inline_keyboard: [
            [
...
export function mediaLibraryKeyboard(keys) {
...
export function llmCategoryKeyboard(categories) {
    const rows = [];
    for (let i = 0; i < categories.length; i += 2) {
        const row = [{ text: `${categories[i].category} (${categories[i].count})`, callback_data: `llm:cat:${categories[i].category}` }];
        if (categories[i + 1])
            row.push({ text: `${categories[i + 1].category} (${categories[i + 1].count})`, callback_data: `llm:cat:${categories[i + 1].category}` });
        rows.push(row);
    }
    rows.push([{ text: '⟵ Admin Menu', callback_data: 'admin:back' }]);
    return { inline_keyboard: rows };
...
export function composeToneKeyboard() {
    return {
        inline_keyboard: [
...
export function composeButtonKeyboard() {
    return {
        inline_keyboard: [
...
export function reviewResultKeyboard() {
