...
function computeATR(candles: AdminCandle[], period: number): number[] {
    const trs: number[] = [];
    for (let i = 1; i < candles.length; i++) {
        const tr = Math.max(
            candles[i].max - candles[i].min,
            Math.abs(candles[i].max - candles[i - 1].close),
            Math.abs(candles[i].min - candles[i - 1].close)
        );
        trs.push(tr);
    }
    const atrs: number[] = [];
...
function computeRSI(candles: AdminCandle[], period: number): number {
...
// ─── MACD ────────────────────────────────────────────────────────────────────
function computeMACD(candles: AdminCandle[]): { macd: number; signal: number; histogram: number } {
...
// ─── Bollinger Bands ────────────────────────────────────────────────────────
function computeBollingerBands(candles: AdminCandle[], period: number = 20, mult: number = 2) {
...
function computeStochastic(candles: AdminCandle[], kPeriod: number = 14, dPeriod: number = 3) {
...
function computeADX(candles: AdminCandle[], period: number = 14): number {
    if (candles.length < period * 2 + 1) return 20; // default to neutral
    const plusDM: number[] = [];
...
// ─── RSI Divergence ──────────────────────────────────────────────────────────
function detectRSIDivergence(candles: AdminCandle[], period: number = 14): { type: 'bullish' | 'bearish' | null; score: number } {
    if (candles.length < period * 3 + 5) return { type: null, score: 0 };
...
// ─── Support/Resistance ──────────────────────────────────────────────────────
function detectSR(candles: AdminCandle[], tolerance: number = 0.002): {
    nearResistance: boolean;
    nearSupport: boolean;
    score: number;
} {
    if (candles.length < 20) return { nearResistance: false, nearSupport: false, score: 0 };
    ...
    if (range === 0) return { nearResistance: false, nearSupport: false, score: 0 };
    ...
    let score = 0;
    if (nearResistance) score = -30; // at resistance → bearish
    if (nearSupport) score = 30; // at support → bullish
    return { nearResistance, nearSupport, score };
}

// ─── Multi-Timeframe Trend ───────────────────────────────────────────────────
function multiTimeframeTrend(candles: AdminCandle[]): { direction: 'call' | 'put' | 'mixed'; score: number } {
    const lookbacks = [10, 20, 30];
    const trends: ('call' | 'put')[] = [];
    for (const lb of lookbacks) {
        if (candles.length < lb + 1) continue;
        ...
        if (ema9[lastIdx] > ema21[lastIdx]) trends.push('call');
        else trends.push('put');
    }
    if (trends.length === 0) return { direction: 'mixed', score: 0 };
    const calls = trends.filter(t => t === 'call').length;
    const puts = trends.length - calls;
    if (calls === trends.length) return { direction: 'call', score: 25 };
    if (puts === trends.length) return { direction: 'put', score: -25 };
    return { direction: 'mixed', score: 0 };
}

// ─── Candlestick Pattern Detection ──────────────────────────────────────────
function detectCandlestickPatterns(candles: AdminCandle[]): { direction: 'call' | 'put' | null; score: number } {
    if (candles.length < 3) return { direction: null, score: 0 };
...
// ─── Volume Surge ────────────────────────────────────────────────────────────
function computeVolumeSurge(candles: AdminCandle[]): { surge: boolean; score: number } {
    if (candles.length < 11 || !candles[0].volume) return { surge: false, score: 0 };
    const lastVol = candles[candles.length - 1].volume ?? 0;
    const avgVol = candles.slice(-11, -1).reduce((s, c) => s + (c.volume ?? 0), 0) / 10;
    if (avgVol === 0) return { surge: false, score: 0 };
...
function computeTrendStrength(candles: AdminCandle[]): { score: number } {
    if (candles.length < 25) return { score: 0 };
    const closes = candles.slice(-20).map(c => c.close);
    const ema = computeEMA(closes, 9);
    // Slope of last 5 EMA values
    const recentEma = ema.slice(-5);
    if (recentEma.length < 2) return { score: 0 };
    let increasing = 0;
    let decreasing = 0;
    for (let i = 1; i < recentEma.length; i++) {
        if (recentEma[i] > recentEma[i - 1]) increasing++;
        else decreasing++;
    }
...
export function runAdminAnalysis(candles: AdminCandle[]): AdminAnalysisResult {
    ...
    if (rsi < 30) { score += 80; reasons.push('RSI oversold'); }
    else if (rsi > 70) { score -= 80; reasons.push('RSI overbought'); }
    else if (rsi < 45) { score += 40; reasons.push('RSI bearish-neutral'); }
    else if (rsi > 55) { score -= 40; reasons.push('RSI bullish-neutral'); }

    // 2. Stochastic (±40)
    const stoch = computeStochastic(sliced, 14, 3);
    if (stoch.k < 20) { score += 40; reasons.push('Stoch oversold'); }
    else if (stoch.k > 80) { score -= 40; reasons.push('Stoch overbought'); }
    else if (stoch.k > stoch.d) { score += 15; reasons.push('Stoch bullish cross'); }
    else if (stoch.k < stoch.d) { score -= 15; reasons.push('Stoch bearish cross'); }
    ...
    if (macd.histogram > 0) { score += 40; reasons.push('MACD bullish'); }
    else { score -= 40; reasons.push('MACD bearish'); }

    // 5. Bollinger Bands (±60)
    const bb = computeBollingerBands(sliced, 20, 2);
    const lastClose = closes[lastIdx];
    if (lastClose <= bb.lower) { score += 60; reasons.push('BB lower band'); }
    else if (lastClose >= bb.upper) { score -= 60; reasons.push('BB upper band'); }
    else if (lastClose < bb.middle) { score += 20; reasons.push('BB below mid'); }
    else { score -= 20; reasons.push('BB above mid'); }
    ...
    score += vol.score;
    if (vol.surge) reasons.push('Volume surge');

    // 9. Candlestick Patterns (±30)
    const pattern = detectCandlestickPatterns(sliced);
    score += pattern.score;
    if (pattern.direction) reasons.push(`Pattern: ${pattern.direction === 'call' ? 'bullish' : 'bearish'}`);

    // 10. Trend Strength (±20)
    const trend = computeTrendStrength(sliced);
    score += trend.score;
    if (trend.score > 0) reasons.push('Trend strong up');
    else if (trend.score < 0) reasons.push('Trend strong down');

    // 11. RSI Divergence (±25)
    const divergence = detectRSIDivergence(sliced, 14);
    if (divergence.type === 'bullish') { score += 25; reasons.push('RSI bullish divergence'); }
    else if (divergence.type === 'bearish') { score -= 25; reasons.push('RSI bearish divergence'); }

    // 12. Multi-Timeframe Trend (±25)
    const mtf = multiTimeframeTrend(sliced);
    score += mtf.score;
    if (mtf.direction === 'call') reasons.push('MTF aligned bullish');
    else if (mtf.direction === 'put') reasons.push('MTF aligned bearish');
    else reasons.push('MTF mixed');

    // 13. Support/Resistance (±30)
    const sr = detectSR(sliced, 0.002);
    score += sr.score;
    if (sr.nearSupport) reasons.push('Near support');
    if (sr.nearResistance) reasons.push('Near resistance');
